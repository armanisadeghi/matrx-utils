...
from database.schema_builder.schema_manager import SchemaManager
...
def example_usage(schema_manager):
    ...
    vcprint(example_column, title="Flashcard ID Column", pretty=True, verbose=generator_verbose, color="cyan")

    example_view = schema_manager.get_view("view_registered_function_all_rels").to_dict()
    vcprint(example_view, title="Full Registered Function View", pretty=True, verbose=generator_verbose, color="yellow")

    example_table = schema_manager.get_table("registered_function").to_dict()
    vcprint(example_table, title="Flashcard History Table", pretty=True, verbose=generator_verbose, color="cyan")
...
def get_full_schema_object(schema, database_project):
    ...
    full_schema_object = {"schema": matrx_schema_entry, "models": matrx_models, "analysis": analysis}
...
if __name__ == "__main__":
    ...
    schema_manager = SchemaManager(schema=schema, database_project=database_project, additional_schemas=additional_schemas, save_direct=save_direct)
    schema_manager.initialize()
    ...
    #
    analysis = schema_manager.analyze_schema()
    vcprint(data=analysis, title="Schema Analysis", pretty=True, verbose=False, color="yellow")
    #
