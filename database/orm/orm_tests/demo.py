...
from common import cool_print
from database.models import DataBroker, DataInputComponent, DataOutputComponent
...
async def basic_operations_demo(broker_id):
    ...
    output_component_object = await broker.fetch_fk("output_component")
    vcprint(output_component_object, title="Output Component", color="bright_purple", pretty=True)
    ...
    vcprint(all_messages_for_all_message_brokers, title="All Messages for All Message Brokers", color="yellow", pretty=True)
    ...
    vcprint(f"Broker cache policy: {DataBroker._cache_policy}", color="bright_lavender", inline=True)
    
    vcprint("\n--> Import placed here to show you never need to think about the cache or state", color="bright_lavender")
    ...
    vcprint(in_cache, title="--> Checking if the broker is in cache... Is it????", color="bright_lavender", inline=True)
    ...
    vcprint(cached_broker_dicts, title="All Cached Brokers", color="bright_pink", pretty=True)
    ...
    vcprint(updated_broker, title="Broker Updated with Input Component", color="bright_lime", pretty=True)
    ...
    updated_broker = await DataBroker.get(id=broker.id, bypass_cache=True)
    vcprint(updated_broker, title="Broker Updated with Multiple Fields", color="bright_lime", pretty=True)
    ...
    updated_broker = await DataBroker.update_fields(
        broker.id,
        data_type="list",
        color="green"
    )
    vcprint(updated_broker, title="Broker Updated Using update_fields", color="bright_lime", pretty=True)
    ...
    vcprint(broker_by_name, title="Attempt to Find Deleted Broker by Name", color="bright_lime", pretty=True)
    ...
    broker_by_id = await DataBroker.get_or_none(id=broker.id)
    vcprint(broker_by_id, title="Attempt to Find Deleted Broker by ID", color="bright_lime", pretty=True)
    ...
    vcprint(" --> Too Lazy to type this really REALLY long and confusing filter?\n", color="bright_gold")
    vcprint("  brokers = await DataBroker.filter(name='New Broker').all()", color="bright_gold")
    ...
    vcprint(f"\nQuery execution time: {duration_ms:.3f}ms (These were not cached)", color="bright_gold")
