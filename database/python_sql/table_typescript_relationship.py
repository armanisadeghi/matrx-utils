    ...
    for table_data in relationships_data:
        ...
        if table_data['referenced_by']:
            for related_table, details in table_data['referenced_by'].items():
                ...
                if (related_table in relationship_map[table_name]["relationships"] and
                        any(r["localField"] == details['foreign_column'] for r in
                            relationship_map[table_name]["relationships"][related_table])):
                    continue
...
def get_ts_object(schema='public', database_project='supabase_automation_matrix', additional_schemas=['auth']):
    ...
    return transform_relationships_for_typescript(relationships, junction_analysis), relationships, junction_analysis, overview_analysis
...
if __name__ == '__main__':
    ...
    vcprint(data=overview_analysis, title='Relationship Overview Analysis', pretty=True, verbose=True, color='yellow')

    vcprint(data=typescript_structure, title='TypeScript Relationship Map', pretty=True, verbose=True, color='cyan')
    vcprint(data=relationships, title='Relationships Data', pretty=True, verbose=True, color='yellow')
