...
def add_sharing_to_table(table_name, schema='public', database_project=None):
    ...
    try:
        ...
        if not result or not result[0]['permissions_exists']:
            vcprint("Error: Permissions table doesn't exist. Run setup_permissions_system first.\n", color='red')
            return False
        ...
        if not result or not result[0]['value_exists']:
            ...
            try:
                ...
                vcprint(f"Error adding {table_name} to resource_type enum: {str(e)}\n", color='red')
                return False
        ...
        try:
            ...
            vcprint(f"Note: No existing update sharing policy to drop: {str(e)}\n", color='yellow')
        ...
        try:
            ...
            vcprint(f"Note: No existing delete sharing policy to drop: {str(e)}\n", color='yellow')
        ...
        if policies and len(policies) > 0:
            vcprint(f"Successfully applied sharing policies to {full_table_name}\n", color='green')
            ...
            if trigger_status and len(trigger_status) > 0:
                ...
                vcprint(f"WARNING: Trigger doesn't appear to be set up correctly for {full_table_name}\n", color='red')
            
            return True
        else:
            vcprint(f"Failed to verify sharing policies for {full_table_name}.\n", color='red')
            return False
    except Exception as e:
