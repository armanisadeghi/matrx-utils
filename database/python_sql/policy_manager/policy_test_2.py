    ...
    try:
        ...
        try:
            ...
            if "no results to fetch" in str(e):
                vcprint(f"RLS likely enabled on {full_table_name} (got expected 'no results to fetch').\n", color='green')
            else:
        ...
        operations = [
            {"name": f"Users can view their own {table_name}", "operation": "SELECT", "check": "USING"},
            {"name": f"Users can insert their own {table_name}", "operation": "INSERT", "check": "WITH CHECK"},
            {"name": f"Users can update their own {table_name}", "operation": "UPDATE", "check": "USING"},
            {"name": f"Users can delete their own {table_name}", "operation": "DELETE", "check": "USING"}
        ]
        ...
        for op in operations:
            ...
            try:
                ...
                if "no results to fetch" in str(e):
                    vcprint(f"Likely dropped policy (got expected 'no results to fetch').\n", color='green')
                else:
            ...
            try:
                ...
                if "no results to fetch" in str(e):
                    vcprint(f"Likely created policy (got expected 'no results to fetch').\n", color='green')
                else:
        ...
        try:
            ...
            if policies and len(policies) > 0:
                ...
                if rls_status and rls_status[0]['relrowsecurity']:
                    vcprint(f"Verified RLS is enabled for {full_table_name}\n", color='green')
                else:
                    vcprint(f"WARNING: RLS might not be enabled for {full_table_name}\n", color='yellow')
                
                return True
            else:
                # Try an alternative approach to check policies
                vcprint("No policies found using pg_policies view, trying alternative query...\n", color='yellow')
                alt_policy_check = """
                SELECT 
                ...
                if alt_policies and len(alt_policies) > 0:
                    vcprint(f"Found {len(alt_policies)} policies using alternative query:\n", color='green')
                    for p in alt_policies:
                        vcprint(f"  - {p['policyname']} on {p['tablename']}\n", color='green')
                    return True
                else:
                    vcprint("No policies found using alternative query method either.\n", color='red')
                    vcprint("This suggests the policies were not created or you don't have permission to view them.\n", color='red')
                    return False
        except Exception as e:
            vcprint(f"Error verifying policies: {str(e)}\n", color='red')
            vcprint("This might be a permissions issue with viewing pg_policies.\n", color='yellow')
            vcprint("The policies may still have been created successfully.\n", color='yellow')
...
if __name__ == "__main__":
    ...
    success = create_supabase_rls_policy(
    ...
    if success:
        ...
        vcprint("You should now check in the Supabase dashboard to confirm the policies are active.\n", color='green')
    else:
