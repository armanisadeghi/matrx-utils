...
def apply_batch_policies(
    ...
    successful_tables = sum(
        1 for table, result in results.items()
        if table != "auth_has_access_function" and (
            (isinstance(result, bool) and result) or
            (isinstance(result, dict) and all(result.values()))
        )
    )
    
    vcprint(f"Successfully applied policies to {successful_tables} of {len(table_names)} tables", color='green')
...
if __name__ == "__main__":
    ...
    vcprint(data=results, title='Batch Policy Results', pretty=True, verbose=True, color='blue')