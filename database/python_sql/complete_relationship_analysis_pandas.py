...
class ColumnBase:
    ...
    @staticmethod
    def analyze_default_value(default_expr: Optional[str]) -> tuple[Optional[str], Optional[str], bool]:
        if not default_expr:
...
class TableBase:
    ...
    schema_structure: Dict[str, Any] = field(default_factory=lambda: {
    ...
    def _analyze_columns(self):
        ...
        for row in results:
            ...
            if table_name in self.analysis:
                ...
                default_value, default_generator_function, is_default_function = (
                    ColumnBase.analyze_default_value(default_expr)
                )
            ...
            for _, row in df.iterrows():
                ...
                if table_name in self.analysis:
                    self.analysis[table_name].metadata = TableMetadata(
                        ...
                        has_timestamps=any(col.base_type == 'timestamp'
                                           for col in self.analysis[table_name].columns.values()),
                        has_soft_delete=any(col.name in ['deleted_at', 'is_deleted']
                                            for col in self.analysis[table_name].columns.values())
                    )
...
def convert_table_analysis_to_dict(table_analysis: TableBase) -> dict:
    ...
    exclusion_info = [{
        ...
        'columns': exc.columns
    } for exc in table_analysis.exclusion_constraints]
    ...
    return {
        ...
        'save_operation_rules': {
            'has_identity': any(col.data_type in ['serial', 'bigserial']
                              for col in table_analysis.columns.values()),
            'has_default_values': [name for name, col in table_analysis.columns.items()
                               if col.default_value is not None],
            'required_columns': [name for name, col in table_analysis.columns.items()
            ...
            'generated_columns': [name for name, col in table_analysis.columns.items()
                              if 'generated' in (col.default_value or '').lower()],
...
if __name__ == '__main__':
    ...
    try:
        ...
        if 'registered_function' in analysis:
            ...
            vcprint(data=registered_function_dict, title='Registered Function Table Analysis', pretty=True,
        ...
        analysis_dict_without_columns = {
            table_name: convert_table_analysis_to_dict_with_columns(table_analysis)
            for table_name, table_analysis in analysis.items()
        }
        ...
        full_analysis_dict = {
            table_name: convert_table_analysis_to_dict(table_analysis)
            for table_name, table_analysis in analysis.items()
        }
