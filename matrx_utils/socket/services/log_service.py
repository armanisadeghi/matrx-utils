...
class LogService(SocketServiceBase):
    ...
    async def mic_check(self):
        await self.stream_handler.send_text_chunk("Mic Check")
        await self.stream_handler.send_text_chunk(f"Message: {str(self.mic_check_message)}")
    ...
    async def read_logs(self):
        ...
        if not os.path.exists(filepath):
            await self.stream_handler.send_text_chunk(f"Error: Log file {self.filename} not found")
            await self.stream_handler.end_stream()
            return
        ...
        try:
            with open(filepath, 'r') as f:
                ...
                # Stream logs as text
                for line in result:
                    await self.stream_handler.send_text_chunk(line)
                await self.stream_handler.end_stream()

        except Exception as e:
            self.logger.error(f"Error reading log file {filepath}: {e}")
            await self.stream_handler.send_text_chunk(f"Error reading logs: {str(e)}")
            await self.stream_handler.end_stream()
    ...
    async def tail_logs(self):
        ...
        if self._tail_active:
            await self.stream_handler.send_text_chunk(f"Existing tailing task for {self.filename} detected, stopping it to start a new one")
        ...
        await self.stream_handler.send_text_chunk(f"Started tailing {self.filename}")
        last_size = 0
        ...
        try:
                ...
                try:
                    with open(filepath, 'r') as f:
                        ...
                        if current_size < last_size:
                            await self.stream_handler.send_text_chunk(f"Log rotation detected for {self.filename}, resetting position")
                            last_size = 0
                    ...
                    await self.stream_handler.send_text_chunk(f"Error: Log file {self.filename} no longer exists, stopping tail")
                    self._tail_active = False
                    break
                except PermissionError:
                    await self.stream_handler.send_text_chunk(f"Error: Permission denied accessing {self.filename}, stopping tail")
                    self._tail_active = False
                    break
                except Exception as e:
                    await self.stream_handler.send_text_chunk(f"Error tailing {self.filename}: {str(e)}")
                    self._tail_active = False
                    break
            ...
            await self.stream_handler.send_text_chunk(f"Unexpected error tailing {self.filename}: {str(e)}")
            self._tail_active = False

        finally:
            if self._tail_active:
                self._tail_active = False
            await self.stream_handler.send_text_chunk(f"Stopped tailing {self.filename}")
            await self.stream_handler.end_stream()

    async def stop_tail_logs(self):
        """Stop the active log tailing task"""
        if not self._tail_active:
            await self.stream_handler.send_text_chunk("No active tailing task to stop")
            await self.stream_handler.end_stream()
            return

        self._tail_active = False
        await self.stream_handler.send_text_chunk("Log tailing stopped")
        await self.stream_handler.end_stream()
        ...
        try:
            ...
            for friendly_name, filepath in LOG_FILE_MAPPING.items():
                ...
                if os.path.isfile(actual_filepath):
                    ...
                    await self.stream_handler.send_text_chunk(f"Warning: Log file {friendly_name} not found at {actual_filepath}")
            if files:
                await self.stream_handler.send_object(files)
            else:
                await self.stream_handler.send_text_chunk("No log files found")
            await self.stream_handler.end_stream()
        except Exception as e:
            self.logger.error(f"Error listing log files: {e}")
            await self.stream_handler.send_text_chunk(f"Error listing log files: {str(e)}")
            await self.stream_handler.end_stream()
