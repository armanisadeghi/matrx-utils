...
class MatrixPrintLog:
    def __init__(self, system_level, class_name, outro=None):
        ...
        # --- THE ONLY CHANGE: Use standard logging ---
        self.logger = logging.getLogger("matrx_utils.vcprint")
        # --- END OF CHANGE ---
    ...
    def get_caller_info(self, number=2):
        ...
        class_name_module = caller_frame.frame.f_globals.get("__name__", "UnknownModule") # Keep original get
        ...
        proper_method_name = self.snake_case_to_title(method_name.split(".")[-1]) # Keep original split
    ...
    def method_into(self, number=1, start_text=""):
        frame = inspect.currentframe()
        caller_frame = inspect.getouterframes(frame)[number]
        method_name = re.sub(r"(?<!^)(?=[A-Z])", " ", caller_frame.function).title() # Keep original regex title case
    ...
    def method_name(self, number=1, pretty=False):
        ...
        method_name = re.sub(r"(?<!^)(?=[A-Z])", " ", caller_frame.function).title() # Keep original regex title case
        if pretty:
            return self.snake_case_to_title(method_name.split(".")[-1]) # Keep original split
        del frame, caller_frame # Delete frame here as original did
        return method_name
    ...
    def get_effective_level(self, message_level=None):
        ...
        if message_level is not None:
            # Keep original logic: use message_level directly if provided
            effective_level = message_level
            # Add check for string type before upper() to avoid errors, likely matching intent
            if isinstance(effective_level, str):
                 effective_level = effective_level.upper()
                 # Fallback if the provided level is invalid
                 if effective_level not in self.level_options:
                      effective_level = self.system_level

        # Keep original override logic, ensuring levels are valid before indexing
        if effective_level in self.level_options and self.override_level in self.level_options:
             ...
             # Handle case where effective_level was invalid from start or became invalid
             effective_level = self.system_level
             if self.level_options.index(effective_level) > self.level_options.index(self.override_level):
                  effective_level = self.override_level
    ...
    def should_log(self, message_level=None):
        ...
        if not self.log_vcprint:
            return False
        # Ensure level is valid before index comparison
        if effective_level not in self.level_options: return False
    ...
    def _clean_for_log(self):
        if isinstance(self.full_text, str):
            self.message_for_log = re.sub(r"[^\x00-\x7F]+", "", self.full_text)
             ...
             self.message_for_log = str(self.full_text) # Ensure string

    def log_message(self):
        try:
            self.logger.info(self.message_for_log)
        except Exception as e:
            # Use ERROR level for the logging error message itself, include traceback
    ...
    def _convert_data(self, data):
        ...
        if data is None:
            ...
            return str(data)
        # Check BaseDTO safely in case import failed
        elif BaseDTO != type(None) and isinstance(data, BaseDTO):
            return data.to_dict()
        elif is_dataclass(data):
             # Check if it's instance, not class type
             if not isinstance(data, type):
                  try: return asdict(data)
                  except TypeError: return str(data) # Fallback
             else: return str(data) # It's the type
        elif hasattr(data, "to_dict") and callable(data.to_dict):
             try: return data.to_dict()
             except Exception: return str(data) # Fallback
    ...
    # --- Public Print Methods --- Keep exactly as provided ---
    def vcprint( self, data=None, title="no title", level=None, color=None, background=None, style=None, pretty=True, indent=4, inline=False, chunks=False, exc_info=None ):
        ...
        self._print_and_or_log( data=data, title=title, level=level, color=color, background=background, style=style, pretty=pretty, indent=indent, inline=inline, chunks=chunks, exc_info=exc_info) # Pass exc_info
    ...
    def _print_and_or_log(
        ...
        inline=None,
        chunks=None,
        exc_info=None, # Keep exc_info
    ):
        # Keep original level processing
        current_level_str = level
        if isinstance(level, str):
             current_level_str = level.upper()
             if current_level_str not in self.level_options:
                  current_level_str = None # Treat invalid string level as None
        ...
        if not should_log and not should_print:
            # Keep original print statement if it was there
        ...
        if title is not None:
            # Keep original logic using get_caller_info(2)[2]
            try:
                 self.title = title if title != "me" else self.get_caller_info(2)[2]
            except IndexError: self.title = title # Fallback

        # Keep original state updates
        if inline is not None: self.inline = inline
        if chunks is not None: self.chunks = chunks
        if color is not None: self.color = color
        if background is not None: self.background = background
        if style is not None: self.style = style
        if pretty is not None: self.pretty = pretty
        if indent is not None: self.indent = indent

        # Keep original full_text construction
        ...
        if should_log:
            self._clean_for_log()
            # Pass exc_info to log_message, although original log_message didn't use it
            self.log_message()
    ...
    def _pretty_print(self):
        ...
        try:
            ...
            # Keep original call to to_matrx_json
            try:
                 ...
                 compact_json_string = re.sub(
                     r"\[\n\s+((?:[\w\d]+(?:,?\s*))+)\n\s+\]", # Use original regex pattern if it worked
            ...
            if self.chunks:
             ...
             if 'frame' in locals() and frame is not None: del frame


    def _colorize(self, text):
        
        COLORS = { "black": "\033[30m", "light_red": "\033[31m", "light_green": "\033[32m", "light_yellow": "\033[33m", "light_blue": "\033[34m", "light_magenta": "\033[35m", "light_cyan": "\033[36m", "gray": "\033[37m", "dark_gray": "\033[90m", "red": "\033[91m", "green": "\033[92m", "yellow": "\033[93m", "blue": "\033[94m", "magenta": "\033[95m", "cyan": "\033[96m", "white": "\033[97m", "bright_orange": "\033[38;5;208m", "bright_pink": "\033[38;5;205m", "pink": "\033[38;5;200m", "bright_purple": "\033[38;5;129m", "bright_lime": "\033[38;5;118m", "bright_teal": "\033[38;5;51m", "bright_lavender": "\033[38;5;183m", "bright_turquoise": "\033[38;5;45m", "bright_gold": "\033[38;5;220m", "bright_silver": "\033[38;5;250m", "bright_red": "\033[38;5;196m", "bright_green": "\033[38;5;46m", "bright_blue": "\033[38;5;27m", "bright_yellow": "\033[38;5;226m", }
        BACKGROUNDS = { "black": "\033[40m", "light_red": "\033[41m", "light_green": "\033[42m", "light_yellow": "\033[43m", "light_blue": "\033[44m", "light_magenta": "\033[45m", "light_cyan": "\033[46m", "gray": "\033[47m", "dark_gray": "\033[100m", "red": "\033[101m", "green": "\033[102m", "yellow": "\033[103m", "blue": "\033[104m", "magenta": "\033[105m", "cyan": "\033[106m", "white": "\033[107m", }
        STYLES = { "bold": "\033[1m", "dim": "\033[2m", "italic": "\033[3m", "underline": "\033[4m", "blink": "\033[5m", "reverse": "\033[7m", "hidden": "\033[8m", "strikethrough": "\033[9m", }
        RESET = "\033[0m"
        ...
        if current_background is None and self.color in ["black", "dark_gray"]:
            ...
            current_style = "reverse" # Keep original reverse style set
    ...
    def print_link(self, path):
        ...
        # from urllib.parse import urlparse # Moved top
        # import os # Moved top
        self.path = path
        if not isinstance(self.path, str): self.path = str(self.path)
        ...
        if parsed_path.scheme and parsed_path.netloc: print(self.path)
        else:
            if not os.path.isabs(self.path): self.path = os.path.abspath(self.path)
