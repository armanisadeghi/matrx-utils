...
Legacy JWT keys (anon / service_role) are NOT accepted. Rotate to the new
API key format (https://supabase.com/docs/guides/getting-started/api-keys).

Only SUPABASE_URL plus at least one new-format key are required.
...
class SupabaseBackend(StorageBackend):
    ...
    def _init_from_settings(self) -> None:
        try:
            ...
            # New-format Supabase API key only. Legacy JWT-style keys
            # (SUPABASE_ANON_KEY / SUPABASE_SERVICE_ROLE_KEY / SUPABASE_MATRIX_KEY)
            # are intentionally not accepted — rotate via
            # https://supabase.com/docs/guides/getting-started/api-keys.
            key: str = (
                self._safe_get(settings, "SUPABASE_SECRET_KEY")
                or self._safe_get(settings, "SUPABASE_MATRIX_SECRET_KEY")
                or self._safe_get(settings, "SUPABASE_PUBLISHABLE_KEY")
                or self._safe_get(settings, "SUPABASE_MATRIX_PUBLISHABLE_KEY")
            )
            if not key:
                return
    ...
    def get_url(
        ...
        *,
        ...
        The ``response_content_disposition`` and ``response_content_type``
        kwargs are accepted for signature parity with S3Backend but are
        not currently forwarded — Supabase storage's signed URL API does
        not expose response-header overrides. Callers that need inline-vs-
        attachment distinction should keep public assets on the S3-backed
        CDN bucket.
        """
        self._require_configured()
        bucket, file_path = self._parse_path(path)
