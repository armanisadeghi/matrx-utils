...
def _data_in_code_dir() -> str:
    override = os.environ.get("MATRX_DATA_IN_CODE_DIR")
    if override:
        return os.path.abspath(override)
    repo_root = os.path.abspath(os.path.join(_HERE, "..", "..", "..", ".."))
    common_dir = os.path.join(repo_root, "common", "utils", "data_in_code")
    if os.path.isfile(os.path.join(common_dir, "current_data.py")):
        return common_dir
    return _HERE


_DATA_DIR = _data_in_code_dir()
DEFAULT_DATA_FILE = os.path.join(_DATA_DIR, "current_data.py")
DEFAULT_TS_DATA_FILE = os.path.join(_DATA_DIR, "current_ts_data.ts")
HISTORY_FILE = os.path.join(_DATA_DIR, "data_history.json")
...
def clean_all_history(verbose: bool = False) -> int:
    from matrx_utils import plt
    ...
    with open(HISTORY_FILE, encoding="utf-8") as f:
        try:
            ...
            history = {}
    trimmed = 0
    for key, entries in list(history.items()):
        if isinstance(entries, list) and len(entries) > 1:
            history[key] = [entries[-1]]
            trimmed += 1
    with open(HISTORY_FILE, "w", encoding="utf-8") as f:
        json.dump(history, f, indent=4)
    if verbose:
        plt(HISTORY_FILE, f"clean_all_history: collapsed {trimmed} key(s) to latest snapshot only")
    return trimmed
