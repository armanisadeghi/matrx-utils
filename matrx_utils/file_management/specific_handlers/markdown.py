# matrx_utils\file_management\specific_handlers\markdown.py
import re
from matrx_utils.file_management.file_handler import FileHandler
...
class MarkdownHandler(FileHandler):
    ...
    # def markdown_to_html(self, root, path): Required this import from markdown2 import markdown.
    #     content = self.read_markdown(root, path)
    #     if content:
    #         if self.xhtml_output:
    #             return markdown(content)
    #         else:
    #             # Convert to HTML 4 Output
    #             return markdown(content).replace("<br />", "<br>").replace("<img />", "<img>")
    #     return None

    # def html_to_markdown(self, root, path): Required this import from markdownify import markdownify
    #     content = self.read_markdown(root, path)
    #     if content:
