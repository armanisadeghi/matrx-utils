...
# ---------------------------------------------------------------------------
# System file access — the EXPLICIT internal-trust marker for the media gate
# ---------------------------------------------------------------------------
#
# The resolve_media ACCESS GATE (file_handling/cloud_sync/_media_resolution.py)
# is fail-CLOSED when no request user is in context: an internal/background
# job (RAG ingest, podcast pipeline, workflow node) that resolves refs it
# constructed server-side must declare itself with this marker, or the gate
# DENIES loudly. Never wrap user-supplied refs in this — if a request user
# exists, the gate authorizes as that user regardless of the marker.

_system_file_access: ContextVar[str | None] = ContextVar(
    "matrx_system_file_access", default=None
)


def get_system_file_access_reason() -> str | None:
    """Return the active system-file-access reason, or None if not marked."""
    return _system_file_access.get(None)


def system_file_access(reason: str):
    """Mark the current task as trusted internal code for file resolution.

    Usage (around a resolve of a server-constructed ref only)::

        with system_file_access("rag-ingest"):
            resolved = await fm.resolve_media_async(ref, needs_bytes=True)
    """
    if not reason or not reason.strip():
        raise ValueError("system_file_access requires a non-empty reason string")
    return _SystemFileAccessBlock(reason.strip())


class _SystemFileAccessBlock:
    def __init__(self, reason: str) -> None:
        self._reason = reason
        self._token: Token | None = None

    def __enter__(self) -> str:
        self._token = _system_file_access.set(self._reason)
        return self._reason

    def __exit__(self, *_: Any) -> None:
        if self._token is not None:
            _system_file_access.reset(self._token)
