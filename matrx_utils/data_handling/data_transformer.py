...
_PLURAL_RULES: list[tuple[str, str]] = [
    ...
    (
        r"(octop|syllab|cact|foc|hippopotam|radi|stimul|alumn|bacill|curr|fung|nucle|termin)i$",
        r"\1i",
    ),
    (
        r"(octop|syllab|cact|foc|hippopotam|radi|stimul|alumn|bacill|curr|fung|nucle|termin)us$",
        r"\1i",
    ),
...
class DataTransformer(metaclass=SingletonMeta):
    def __init__(self):
        ...
        self.sql_to_typescript_types = {
            ...
            "vector": "number[]",  # pgvector embedding — array of floats on the wire
            "boolean": "boolean",
            "smallint": "number",
            "bigint": "number",
            "integer": "number",
            ...
            # These are what pg_type.typname / pg_attribute.atttypid resolve to for arrays;
            # information_schema surfaces the same arrays as e.g. "integer[]" (handled above).
            "_bpchar": "string[]",
            "_bool": "boolean[]",
            "_int2": "number[]",
            "_int4": "number[]",
            ...
            "_cidr": "string[]",
            "_macaddr": "string[]",
            "_hstore": "Record<string, string>[]",
            # Postgres range types — sent as canonical text form ("[lo,hi)") over JSON.
            "int4range": "string",
            "int8range": "string",
            "numrange": "string",
            "tsrange": "string",
        ...
        self.sql_to_matrx_schema = {
            ...
            "vector": "number",  # pgvector embedding — closest scalar matrx schema type
            "boolean": "boolean",
            "smallint": "number",
            "bigint": "number",
            "integer": "number",
            ...
            # Postgres internal array type codes (leading underscore = array of base type).
            # Mirror sql_to_python_models_field so base_type lookups never miss.
            "_bpchar": "string[]",
            "_bool": "boolean[]",
            "_int2": "number[]",
            "_int4": "number[]",
            ...
            "_macaddr": "string[]",
            "_hstore": "object[]",
            # Postgres range types — surfaced as canonical text form.
            "int4range": "string",
            "int8range": "string",
            "numrange": "string",
            "tsrange": "string",
        ...
        self.sql_to_python_models_field = {
            ...
            "jsonb": "JSONBField",
            "json": "JSONField",
            "vector": "VectorField",  # pgvector — emitted as VectorField(dimensions=N) by columns.py
            ...
            # OID + OID-alias reference types (pg_catalog). These surface as the
            # qualified object name (text) when selected, so TextField is the safe,
            # read-faithful representation; "oid" itself is an unsigned int.
            "regclass": "TextField",
            "regproc": "TextField",
            "regprocedure": "TextField",
            "regoper": "TextField",
            "regoperator": "TextField",
            "regtype": "TextField",
            "regrole": "TextField",
            "regnamespace": "TextField",
            "regconfig": "TextField",
            "regdictionary": "TextField",
            "oid": "IntegerField",
            ...
            # Postgres range types → RangeField. Renderer in
            # matrx_orm.schema_builder.columns must special-case RangeField to
            # pass the required positional `range_type` arg.
            "int4range": "RangeField",
            "int8range": "RangeField",
            "numrange": "RangeField",
            "tsrange": "RangeField",
            "tstzrange": "RangeField",
    ...
    def set_enum_list(self, enum_list):
        vcprint(
            data=enum_list,
            title="Enum list updated with the following values",
        ...
        for enum in self.enum_list:
            for mapping_name, enum_type in mapping_configs.items():
                ...
                if mapping_dict is not None:
                    ...
                    vcprint(
                        data={
                            "error": "Missing mapping dictionary",
    ...
    def to_python_models_field(self, s):
        ...
        # Hard fail. The previous behavior returned a comment-fragment string
        # ("Any # ERROR! ...") that silently corrupted the generated module
        # (broke the imports line via embedded '#', and produced an unparseable
        # `field = Any # ... ()` assignment). Surface unmapped types loudly so
        # the schema generator aborts before writing a poisoned file.
        vcprint(f"ERROR! Python Model Field Not Found! Original Value: {s}", color="red")
        raise ValueError(
            f"DataTransformer: no matrx-orm Field is mapped for Postgres type "
            f"{s!r}. Add an entry to "
            f"DataTransformer.sql_to_python_models_field in "
            f"matrx_utils/data_handling/data_transformer.py."
        )
        ...
        try:
            if not data_type or data_type.lower() in ["blank", "null", "any", "unknown", ""]:
                ...
                return (
                    json.loads(value)
                    if isinstance(value, str)
                    else (value if isinstance(value, dict) else json.dumps(value))
                )
            elif data_type == "list":
                return (
                    json.loads(value)
                    if isinstance(value, str)
                    else (value if isinstance(value, list) else json.dumps(value))
                )
            elif data_type == "str":
                return str(value)
    ...
    def python_dict_to_ts_with_updates(
        self, name, obj, keys_to_camel=True, export=True, as_const=False, ts_type=None, indent=0
    ):
        if keys_to_camel:
...
if __name__ == "__main__":
    ...
    print(
        f"to_type_annotation (TypeScript): {transformer.to_type_annotation('integer', 'typescript')}"
    )
