...
def get_stale_statistics(schema="public", days_threshold=7):
    ...
    return sorted(
        stale_tables,
        key=lambda x: x["rows"] if x["rows"] is not None else 0,
        reverse=True,
    )
...
if __name__ == "__main__":
    ...
    vcprint(
        data=get_tables_and_views_list(schema),
        title="All Tables and Views (Simple List)",
        pretty=True,
        verbose=True,
        color="blue",
    )
    vcprint(
        data=get_tables_and_views_list(schema, details=True),
        title="All Tables and Views (Detailed)",
        pretty=True,
        verbose=True,
        color="blue",
    )
    ...
    vcprint(
        data=get_tables(schema),
        title="Tables Only (Simple List)",
        pretty=True,
        verbose=True,
        color="green",
    )
    vcprint(
        data=get_tables(schema, details=True),
        title="Tables Only (Detailed)",
        pretty=True,
        verbose=True,
        color="green",
    )
    ...
    vcprint(
        data=get_views(schema),
        title="Views Only (Simple List)",
        pretty=True,
        verbose=True,
        color="yellow",
    )
    vcprint(
        data=get_views(schema, details=True),
        title="Views Only (Detailed)",
        pretty=True,
        verbose=True,
        color="yellow",
    )
    ...
    vcprint(
        data=get_row_analysis(schema),
        title="Tables Potentially Needing Indexing",
        pretty=True,
        verbose=True,
        color="red",
    )
    vcprint(
        data=get_stale_statistics(schema),
        title="Tables with Potentially Stale Statistics",
        pretty=True,
        verbose=True,
        color="magenta",
    )
