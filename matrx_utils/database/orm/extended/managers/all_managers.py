...
from dataclasses import dataclass
from database.orm.core.extended import BaseManager, BaseDTO
from database.orm.models import UserTables
from typing import Optional, Type, Any
from common import vcprint
...
class UserTablesDTO(BaseDTO):
    ...
    async def _initialize_dto(self, user_tables_item):
        '''Override the base initialization method.'''
        self.id = str(user_tables_item.id)
        await self._process_core_data(user_tables_item)
        await self._process_metadata(user_tables_item)
        await self._initial_validation(user_tables_item)
        self.initialized = True

    async def _process_core_data(self, user_tables_item):
        '''Process core data from the model item.'''
        pass

    async def _process_metadata(self, user_tables_item):
        '''Process metadata from the model item.'''
        pass

    async def _initial_validation(self, user_tables_item):
        '''Validate fields from the model item.'''
        pass
    ...
    async def get_validated_dict(self):
        ...
        dict_data = self.to_dict()
        if not validated:
            vcprint(dict_data, "[UserTablesDTO] Validation Failed", verbose=True, pretty=True, color="red")
        return dict_data



class UserTablesBase(BaseManager):
    def __init__(self, dto_class: Optional[Type[Any]] = None):
        self.dto_class = dto_class or UserTablesDTO
        super().__init__(UserTables, self.dto_class)
...
from dataclasses import dataclass
from database.orm.core.extended import BaseManager, BaseDTO
from database.orm.models import ScrapeBaseConfig
from typing import Optional, Type, Any
from common import vcprint
...
class ScrapeBaseConfigDTO(BaseDTO):
    ...
    async def _initialize_dto(self, scrape_base_config_item):
        '''Override the base initialization method.'''
        self.id = str(scrape_base_config_item.id)
        await self._process_core_data(scrape_base_config_item)
        await self._process_metadata(scrape_base_config_item)
        await self._initial_validation(scrape_base_config_item)
        self.initialized = True

    async def _process_core_data(self, scrape_base_config_item):
        '''Process core data from the model item.'''
        pass

    async def _process_metadata(self, scrape_base_config_item):
        '''Process metadata from the model item.'''
        pass

    async def _initial_validation(self, scrape_base_config_item):
        '''Validate fields from the model item.'''
        pass
    ...
    async def get_validated_dict(self):
        ...
        dict_data = self.to_dict()
        if not validated:
            vcprint(dict_data, "[ScrapeBaseConfigDTO] Validation Failed", verbose=True, pretty=True, color="red")
        return dict_data



class ScrapeBaseConfigBase(BaseManager):
    def __init__(self, dto_class: Optional[Type[Any]] = None):
        self.dto_class = dto_class or ScrapeBaseConfigDTO
        super().__init__(ScrapeBaseConfig, self.dto_class)
    ...
    async def create_scrape_base_config(self, **data):
        return await self.create_item(**data)

    async def delete_scrape_base_config(self, id):
        return await self.delete_item(id)

    async def get_scrape_base_config_with_all_related(self, id):
        return await self.get_item_with_all_related(id)

    async def load_scrape_base_config_by_id(self, id):
        return await self.load_by_id(id)

    async def load_scrape_base_config(self, use_cache=True, **kwargs):
        return await self.load_item(use_cache, **kwargs)

    async def update_scrape_base_config(self, id, **updates):
        return await self.update_item(id, **updates)

    async def exists(self, id):
        return await self.exists(id)

    async def load_scrape_base_configs(self, **kwargs):
        return await self.load_items(**kwargs)

    async def filter_scrape_base_configs(self, **kwargs):
        return await self.filter_items(**kwargs)

    async def get_or_create(self, defaults=None, **kwargs):
        return await self.get_or_create(defaults, **kwargs)

    async def load_scrape_base_configs_by_ids(self, ids):
        return await self.load_items_by_ids(ids)
    ...
    @property
    def active_scrape_base_config_ids(self):
        return self.active_item_ids



class ScrapeBaseConfigManager(ScrapeBaseConfigBase):
    _instance = None

    def __new__(cls, *args, **kwargs):
        if cls._instance is None:
            cls._instance = super(ScrapeBaseConfigManager, cls).__new__(cls)
        return cls._instance

    def __init__(self):
        super().__init__()

scrape_base_config_manager_instance = ScrapeBaseConfigManager()
...
from dataclasses import dataclass
from database.orm.core.extended import BaseManager, BaseDTO
from database.orm.models import TableData
from typing import Optional, Type, Any
from common import vcprint
...
class TableDataBase(BaseManager):
    def __init__(self, dto_class: Optional[Type[Any]] = None):
...
from dataclasses import dataclass
from database.orm.core.extended import BaseManager, BaseDTO
from database.orm.models import TableFields
from typing import Optional, Type, Any
from common import vcprint
...
class TableFieldsDTO(BaseDTO):
    ...
    async def _initialize_dto(self, table_fields_item):
        '''Override the base initialization method.'''
        self.id = str(table_fields_item.id)
        await self._process_core_data(table_fields_item)
        await self._process_metadata(table_fields_item)
        await self._initial_validation(table_fields_item)
        self.initialized = True

    async def _process_core_data(self, table_fields_item):
        '''Process core data from the model item.'''
        pass

    async def _process_metadata(self, table_fields_item):
        '''Process metadata from the model item.'''
        pass

    async def _initial_validation(self, table_fields_item):
        '''Validate fields from the model item.'''
        pass
    ...
    async def get_validated_dict(self):
        ...
        dict_data = self.to_dict()
        if not validated:
            vcprint(dict_data, "[TableFieldsDTO] Validation Failed", verbose=True, pretty=True, color="red")
        return dict_data



class TableFieldsBase(BaseManager):
    def __init__(self, dto_class: Optional[Type[Any]] = None):
        self.dto_class = dto_class or TableFieldsDTO
        super().__init__(TableFields, self.dto_class)
    ...
    async def create_table_fields(self, **data):
        return await self.create_item(**data)

    async def delete_table_fields(self, id):
        return await self.delete_item(id)

    async def get_table_fields_with_all_related(self, id):
        return await self.get_item_with_all_related(id)

    async def load_table_fields_by_id(self, id):
        return await self.load_by_id(id)

    async def load_table_fields(self, use_cache=True, **kwargs):
        return await self.load_item(use_cache, **kwargs)

    async def update_table_fields(self, id, **updates):
        return await self.update_item(id, **updates)

    async def exists(self, id):
        return await self.exists(id)

    async def load_table_field(self, **kwargs):
        return await self.load_items(**kwargs)

    async def filter_table_field(self, **kwargs):
        return await self.filter_items(**kwargs)

    async def get_or_create(self, defaults=None, **kwargs):
        return await self.get_or_create(defaults, **kwargs)

    async def get_table_fields_with_user_tables(self, id):
        return await self.get_item_with_related(id, 'user_tables')

    async def get_table_field_with_user_tables(self):
        return await self.get_items_with_related('user_tables')

    async def load_table_field_by_table_id(self, table_id):
        return await self.load_items(table_id=table_id)

    async def filter_table_field_by_table_id(self, table_id):
        return await self.filter_items(table_id=table_id)

    async def load_table_field_by_data_type(self, data_type):
        return await self.load_items(data_type=data_type)

    async def filter_table_field_by_data_type(self, data_type):
        return await self.filter_items(data_type=data_type)

    async def load_table_field_by_user_id(self, user_id):
        return await self.load_items(user_id=user_id)

    async def filter_table_field_by_user_id(self, user_id):
        return await self.filter_items(user_id=user_id)

    async def load_table_field_by_ids(self, ids):
        return await self.load_items_by_ids(ids)
    ...
    @property
    def active_table_fields_ids(self):
        return self.active_item_ids



class TableFieldsManager(TableFieldsBase):
    _instance = None

    def __new__(cls, *args, **kwargs):
        if cls._instance is None:
            cls._instance = super(TableFieldsManager, cls).__new__(cls)
        return cls._instance

    def __init__(self):
        super().__init__()

table_fields_manager_instance = TableFieldsManager()
