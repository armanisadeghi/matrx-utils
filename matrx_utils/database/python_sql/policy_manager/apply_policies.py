...
def apply_policies(table_name: str, database_project: str, policy_types: Optional[List[str]] = None) -> Dict[str, Union[bool, Dict[str, bool]]]:
    """
    ...
    all_success = all(success if isinstance(success, bool) else all(success.values()) for success in results.values())

    if all_success:
        vcprint(f"Successfully applied all policies to {table_name}!", color="green")
    else:
        vcprint(
            f"Some policies failed to apply to {table_name}. Check results for details.",
            color="yellow",
        )
...
if __name__ == "__main__":
    ...
    vcprint(
        data=results,
        title=f"Policy Results for {table_name} ONLY",
        pretty=True,
        verbose=True,
        color="blue",
    )
